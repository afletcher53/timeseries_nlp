...
def main():
    ...
    dataset, labels = w1.split_window(time_series_df)
    vectorised_data = [list(map(text_vectorization, x)) for x in dataset]
    vectorised_data = numpy.array(vectorised_data)
    X_train, X_test, Y_train, Y_test = train_test_split(
        vectorised_data, labels, test_size=0.2
    )
